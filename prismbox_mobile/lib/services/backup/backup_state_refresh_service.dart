...
class BackupStateRefreshService {
  ...
  BackupStateRefreshService({
    ...
    this.autoRefreshOnStart = true,
           ...
           refreshInterval ?? TaskConfig.backupStateRefreshInterval;
  ...
  Future<void> start(String userId, BackupStateNotifier notifier) async {
    ...
    if (autoRefreshOnStart) {
      try {
        ...
        _logger.warning('Failed to refresh on start', e, stackTrace);
    ...
    _refreshTimer = Timer.periodic(refreshInterval, (_) async {
      ...
      try {
        ...
        // 如果连续失败太多次，停止刷新
        if (_consecutiveFailures >= TaskConfig.maxConsecutiveFailures) {
          _logger.severe('Too many consecutive failures, stopping refresh');
          stop();
        }
      }
    });
