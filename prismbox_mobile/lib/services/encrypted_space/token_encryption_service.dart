// lib/services/encrypted_space/token_encryption_service.dart
...
class TokenEncryptionService {
  static final TokenEncryptionService _instance = TokenEncryptionService._internal();
  factory TokenEncryptionService() => _instance;
  TokenEncryptionService._internal();

  final Logger _log = Logger('TokenEncryptionService');
  ...
  String encryptToken({
    ...
    try {
      ...
      // 组合IV、密文和HMAC
      final result = Uint8List(iv.length + tokenBytes.length + digest.bytes.length);
      result.setRange(0, iv.length, iv);
      result.setRange(iv.length, iv.length + tokenBytes.length, tokenBytes);
      result.setRange(iv.length + tokenBytes.length, result.length, digest.bytes);
