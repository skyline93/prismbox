// lib/services/sync/asset_sync_service.dart
...
class AssetSyncService {
  ...
  AssetSyncService({required AppDatabase database, ApiService? apiService})
    : _database = database,
  ...
  /// 获取远程资产详细信息
  Future<Map<String, dynamic>?> _getRemoteAssetInfo(
  ...
  Future<void> syncAssetAfterUpload(UploadTaskEntityData task) async {
    try {
      ...
      if (localAsset == null) {
        _logger.warning('Local asset not found: assetId=${task.assetId}');
        return;
      }
      ...
      if (remoteAssetInfo == null) {
        _logger.warning('Remote asset not found for checksum: $checksum');
      ...
      final remoteAssetData = RemoteAssetEntityData(
        ...
        ownerId: userId,
        name:
            remoteAssetInfo['original_filename'] as String? ??
            remoteAssetInfo['filename'] as String? ??
            '',
        type: _convertItemTypeToAssetType(
          remoteAssetInfo['item_type'] as String,
        ),
