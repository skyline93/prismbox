...
class BackgroundSyncResult {
  ...
  factory BackgroundSyncResult.combine(
    BackgroundSyncResult a,
    BackgroundSyncResult b,
  ) {
...
class BackgroundSyncManager {
  ...
  Future<BackgroundSyncResult> syncAll({
    ...
    try {
      // 阶段一：本地同步
      final localResult = await syncLocal(cancellationToken: cancellationToken);
    ...
    try {
      ...
      while (hasMore && !cancellationToken.isCancelled) {
        try {
          ...
          if (response.statusCode != null &&
            ...
            for (final item in items) {
              ...
              try {
                ...
                // 检查本地是否已存在
                final existing = await _database.remoteAssetDao.getAssetById(
                  remoteAsset.id,
                );
          ...
          _logger.warning('Failed to fetch page $page: $e', e, stackTrace);
        ...
        await Future.wait(
          batch.map((assetId) async {
            try {
              ...
              // 验证文件
              final validation = await FileHashUtil.validateFileForHashing(
                asset.path,
              );
