...
class ThumbnailApiImpl: ThumbnailApi {
  ...
  func getThumbhash(thumbhash: String, completion: @escaping (Result<[String : Int64], any Error>) -> Void) {
    Self.processingQueue.async {
      ...
      else { return completion(.failure(PigeonError(code: "INVALID_BASE64", message: "Invalid base64 string: \(thumbhash)", details: nil)))}
  ...
  func requestImage(assetId: String, requestId: Int64, width: Int64, height: Int64, isVideo: Bool, completion: @escaping (Result<[String: Int64], any Error>) -> Void) {
    ...
    let item = DispatchWorkItem {
      ...
      guard let asset = Self.requestAsset(assetId: assetId)
        ...
        completion(.failure(PigeonError(code: "ASSET_NOT_FOUND", message: "Could not get asset data for \(assetId)", details: nil)))
        return
      }
      ...
      Self.imageManager.requestImage(
        ...
        resultHandler: { (_image, info) -> Void in
          ...
          if let error = info?[PHImageErrorKey] as? Error {
            imageError = error
            print("[ThumbnailApi] PHImageManager error for asset \(assetId): \(error.localizedDescription)")
          }
          
          // 检查是否被取消
          if let cancelled = info?[PHImageCancelledKey] as? Bool, cancelled {
            isCancelled = true
            print("[ThumbnailApi] Image request cancelled for asset \(assetId)")
          }
          
          // 检查是否是降级版本
          if let degraded = info?[PHImageResultIsDegradedKey] as? Bool, degraded {
            isDegraded = true
            print("[ThumbnailApi] Image is degraded for asset \(assetId)")
          }
          ...
          // 记录详细信息
          if _image == nil {
            print("[ThumbnailApi] PHImageManager returned nil image for asset \(assetId), error: \(imageError?.localizedDescription ?? "unknown"), cancelled: \(isCancelled), degraded: \(isDegraded)")
          } else if _image?.cgImage == nil {
            print("[ThumbnailApi] PHImageManager returned image without CGImage for asset \(assetId)")
          } else {
            print("[ThumbnailApi] PHImageManager succeeded for asset \(assetId), size: \(_image?.size ?? CGSize.zero), cgImage size: \(_image?.cgImage?.width ?? 0)x\(_image?.cgImage?.height ?? 0)")
          }
        }
      )
      ...
      if let error = imageError {
        Self.removeRequest(requestId: requestId)
        return completion(.failure(PigeonError(code: "IMAGE_DECODE_FAILED", message: "PHImageManager error for \(assetId): \(error.localizedDescription)", details: nil)))
      }
      
      guard let image = image,
            let cgImage = image.cgImage else {
        Self.removeRequest(requestId: requestId)
        let errorMsg = imageError != nil ? "PHImageManager error: \(imageError!.localizedDescription)" : "Could not get pixel data"
        print("[ThumbnailApi] Failed to get image/CGImage for asset \(assetId): \(errorMsg)")
        return completion(.failure(PigeonError(code: "IMAGE_DECODE_FAILED", message: "\(errorMsg) for \(assetId)", details: nil)))
      }
      ...
      guard let context = CGContext(
        ...
        pointer.deallocate()
        Self.removeRequest(requestId: requestId)
        return completion(.failure(PigeonError(code: "CONTEXT_CREATION_FAILED", message: "Could not create context for \(assetId)", details: nil)))
      }
