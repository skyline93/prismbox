...
/// 显示收藏的媒体资源，支持本地/远程切换
@RoutePage()
class FavoriteTimelinePage extends ConsumerStatefulWidget {
  const FavoriteTimelinePage({super.key});

  @override
  ConsumerState<FavoriteTimelinePage> createState() =>
      _FavoriteTimelinePageState();
}

class _FavoriteTimelinePageState extends ConsumerState<FavoriteTimelinePage>
    ...
    final timelineSectionsAsync = permissionAsync.when(
      data: (permissionState) {
        if (permissionState is PhotoPermissionGranted) {
          return ref.watch(timelineSectionsProvider(pageId: 'favorite'));
        }
              ...
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  if (isSelectionActive)
                    const TimelineSelectionAppBar(pageId: 'favorite')
                  else
