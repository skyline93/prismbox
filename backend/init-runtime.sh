...
if [ ! -f "data/configs/config.yaml" ]; then
    echo "提示: data/configs/config.yaml 不存在，容器启动时会自动生成（使用环境变量配置）"
fi
...
if [ "${ALBUM_ENABLE_HTTPS:-false}" = "true" ]; then
    if [ ! -f "data/cert/cert.pem" ] || [ ! -f "data/cert/key.pem" ]; then
        ...
        echo ""
        echo "选项 2: 手动放置证书文件"
        echo "  请将证书文件放置到 data/cert/ 目录下："
        echo "    - data/cert/cert.pem"
        echo "    - data/cert/key.pem"
    fi
fi
...
echo "2. （可选）如果启用 HTTPS，请将 SSL 证书放置到 data/cert/ 目录"
