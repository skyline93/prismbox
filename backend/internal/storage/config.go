...
// Config 存储配置。默认生成配置不包含 Secondary，仅代码内保留类型与工厂占位。
type Config struct {
	Primary   *PrimaryStorageConfig   `yaml:"primary" mapstructure:"primary"`
	Secondary *SecondaryStorageConfig `yaml:"secondary,omitempty" mapstructure:"secondary"` // 默认不生成，omitempty
}
...
// DataDir 为工作根目录，仅用于 temp、staging、cache 的默认父目录；池内文件路径由 pool.Path + key 决定，池根来自 DB（storage_pools）的 location。
...
// PoolManagerConfig 存储池管理器配置。高级参数：flush_interval、reconcile_interval 等默认见 loader defaultConfig，调优见文档。
...
type PerformanceConfig struct {
	...
	CachePath       string           `yaml:"cache_path" mapstructure:"cache_path"` // 磁盘缓存根目录，未配置时使用默认或仅内存缓存
	...
	ReadBufferSize  types.Size     `yaml:"read_buffer_size" mapstructure:"read_buffer_size"`
	WriteBufferSize types.Size     `yaml:"write_buffer_size" mapstructure:"write_buffer_size"`
}
