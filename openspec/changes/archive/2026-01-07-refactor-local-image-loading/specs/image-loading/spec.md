...
系统 SHALL 实现混合架构的本地图片加载策略，缩略图和小尺寸预览使用原生层解码，全尺寸原图使用 Flutter 层处理。

#### Scenario: 缩略图使用原生解码
- **WHEN** 请求加载本地图片缩略图（`isThumbnail=true` 或尺寸小于阈值）
...
#### Scenario: 原图使用 Flutter 层处理
- **WHEN** 请求加载本地图片原图（尺寸大于阈值或明确请求原图）
- **THEN** 系统 SHALL 使用 `ImmutableBuffer.fromFilePath` 直接读取文件
- **AND** 系统 SHALL 使用 Flutter 的 `ImageDecoderCallback` 解码
- **AND** 系统 SHALL 避免跨层大文件数据传递，提升内存效率

#### Scenario: 尺寸阈值决策
- **WHEN** 请求加载本地图片
- **THEN** 系统 SHALL 根据请求尺寸和类型决定解码路径
- **AND** 如果 `isThumbnail=true`，系统 SHALL 强制使用原生解码
- **AND** 如果尺寸小于 512x512，系统 SHALL 使用原生解码
- **AND** 如果尺寸大于等于 512x512，系统 SHALL 使用 Flutter 层处理
- **AND** 阈值 SHALL 可配置（通过常量定义）

#### Scenario: 渐进式加载保持不变
- **WHEN** 使用 `LocalFullImageProvider` 加载图片
- **THEN** 系统 SHALL 保持渐进式加载流程（缩略图 → 适配尺寸 → 原图）
- **AND** 缩略图阶段 SHALL 使用原生解码（通过 `LocalThumbProvider`）
- **AND** 适配尺寸阶段 SHALL 根据尺寸决定使用原生或 Flutter 层
- **AND** 原图阶段 SHALL 使用 Flutter 层处理（`ImmutableBuffer.fromFilePath`）
