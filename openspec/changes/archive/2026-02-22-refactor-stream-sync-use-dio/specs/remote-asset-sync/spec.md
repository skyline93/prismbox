...
系统 SHALL 提供远程资产同步功能，从服务器同步远程媒体资源到本地数据库。远程资产同步 SHALL 完全独立于本地资产同步，不进行任何关联。客户端 SHALL 使用与其它 API 请求相同的 HTTP 客户端（Dio）发起流式同步与 checkpoint 请求，并享有统一的认证与 401 自动刷新重试行为。
...
#### Scenario: 流式同步使用统一 API 客户端与认证
- **WHEN** 客户端发起流式同步请求（POST /api/v1/sync/assets/stream）或服务端 checkpoint 上报（POST /api/v1/sync/checkpoint）
- **THEN** 客户端 SHALL 使用与其它 API 相同的 Dio 实例与认证头（内存中的 access token）
- **AND** 若服务端返回 401，客户端 SHALL 通过现有错误拦截器尝试刷新 token 并自动重试该请求
- **AND** 客户端 SHALL 不再使用原生 http 包或仅从 Store 读取 token 的方式发起上述请求
