...
- **AND** 系统 SHALL 在执行删除后立即刷新时间线数据，确保 UI 立即更新
- **AND** 系统 SHALL 同时 invalidate `timelineAssetsProvider()` 和 `timelineSectionsProvider`，确保依赖链上的所有 provider 都刷新
...
- **AND** 系统 SHALL 在删除完成后同时 invalidate `timelineAssetsProvider()` 和 `timelineSectionsProvider`，确保照片页面立即更新
