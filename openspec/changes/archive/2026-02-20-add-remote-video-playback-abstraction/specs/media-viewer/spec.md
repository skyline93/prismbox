...
- **AND** 类 SHALL 通过播放后端工厂（如 PlaybackBackendFactory）获取实现 ViewerPlaybackController 的控制器，并按 assetId 缓存
...
- **AND** 组件 SHALL 通过 ViewerVideoManager 获取 ViewerPlaybackController（由播放后端工厂按源类型选择引擎）
- **AND** 组件 SHALL 使用控制器的 buildVideoView() 渲染视频画面，并依赖控制器接口处理加载与错误状态
- **AND** 组件 SHALL 集成视频控制器 UI（控制栏接收同一 ViewerPlaybackController 接口）
...
- **AND** 组件 SHALL 接受 ViewerPlaybackController 类型的参数，仅通过该接口读取 position、duration、isPlaying 并调用 play、pause、seekTo、setVolume
- **AND** 组件 SHALL 封装视频播放控制逻辑（播放/暂停、进度条、静音等）
- **AND** 组件 SHALL 使用 `ValueListenableBuilder` 或接口的监听器优化性能
...
媒体查看器在播放仅远程存在的视频资产时，SHALL 能够正常加载并播放；实现方式 SHALL 符合 video-playback 能力规格（使用支持自定义 HTTP 头的播放引擎并携带认证头），以便请求服务端下载接口时通过鉴权。

#### Scenario: 仅远程视频在查看器中可播
- **WHEN** 用户在媒体查看器中打开仅远程存在的视频（无本地文件）
